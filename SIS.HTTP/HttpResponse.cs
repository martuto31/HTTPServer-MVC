...
namespace SIS.HTTP
{
    public class HttpResponse
    {
        public HttpResponse(HttpResponseCode statusCode, byte[] body)
        {
            ...
            //else if (body.Length == 0 || body == null)
            //{
            //    this.Headers.Add(new Header("Content-Length", body.Length.ToString()));
            //}
        }
        ...
        public override string ToString()
        {
            ...
            responseAsString.Append($"{httpVersionAsString} {(int)this.StatusCode} {this.StatusCode}"+ Environment.NewLine);
            foreach (var header in this.Headers)
            {
                responseAsString.Append(header.ToString() + Environment.NewLine);
            }
            foreach (var cookie in this.Cookies)
            {
                responseAsString.Append("Set-Cookie: " + cookie.ToString() + Environment.NewLine);
            }
            responseAsString.Append(Environment.NewLine); // HttpConst.NewLine
